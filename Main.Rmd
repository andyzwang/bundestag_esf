...
The goal of this project is to collate individual ESF+ projects into German electoral districts, and to standardize formatting between the various data sources from each individual federal state (*Bundesland*). 
...
While it is expected that most BWK encompass multiple PLZ, with the average BWK corresponding to `r pull( wk_plz_number)` districts. However, `r pull(plz_multiple_wk)` PLZs correspond to multiple BWK, with the most extreme being PLZ 10178 in Berlin, which corresponds to 12 electoral districts. As such, matching ESP projects on PLZ will lead to a certain margin of error, and the column `mult_wk` is set to `TRUE` for PLZ that have multiple BWK. 
